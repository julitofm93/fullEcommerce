...
export default class SessionService{
    ...
    login = ({body,callbackSuccess,callbackError}) => {
        const data = {url:"http://localhost:8081/api/auth/login",config:getAuthHeaders(),body,callbackSuccess,callbackError};
    ...
    register = ({body,callbackSuccess,callbackError})=>{
        const data = {url:"http://localhost:8081/api/auth/register",config:getAuthHeaders(),body,callbackSuccess,callbackError};
        this.httpClient.makePostRequest(data);
    }
}